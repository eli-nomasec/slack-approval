...
const inputsFromEnv = {
    ...
    'auto-approve-timeout': process.env.AUTO_APPROVE_TIMEOUT || "10" // Defaults to 5 seconds (300 seconds)
};
const getInput = (name) => isProd ? core.getInput(name) : inputsFromEnv[name];
async function run() {
    try {
        ...
        const approvalPromise = new Promise((resolve) => {
            app.action("slack-approval-approve", async ({ ack, say }) => {
                await ack();
                approvalStatus = "approved";
                await say("Deployment approved.");
                resolve("approved");
            });
            app.action("slack-approval-reject", async ({ ack, say }) => {
                await ack();
